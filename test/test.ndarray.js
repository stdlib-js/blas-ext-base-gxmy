...
tape( 'the function has an arity of 7', function test( t ) {
	t.strictEqual( gxmy.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'the function multiplies elements of `x` by the corresponding elements of `y` and assigns the results to `y` (accessors)', function test( t ) {
	...
	gxmy( x.length, toAccessorArray( x ), 1, 0, toAccessorArray( y ), 1, 0 );
	t.deepEqual( y, expected, 'returns expected value' );
...
tape( 'the function supports an `x` stride (accessors)', function test( t ) {
	...
	gxmy( 3, toAccessorArray( x ), 2, 0, toAccessorArray( y ), 1, 0 );
	t.deepEqual( y, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports a `y` stride (accessors)', function test( t ) {
	...
	gxmy( 3, toAccessorArray( x ), 1, 0, toAccessorArray( y ), 2, 0 );
	t.deepEqual( y, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports negative strides', function test( t ) {
	...
	gxmy( 3, x, -2, x.length-1, y, -1, y.length-1 );
	t.deepEqual( y, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports negative strides (accessors)', function test( t ) {
	...
	gxmy( 3, toAccessorArray( x ), -2, x.length-1, toAccessorArray( y ), -1, y.length-1 ); // eslint-disable-line max-len
	t.deepEqual( y, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports an `x` offset (accessors)', function test( t ) {
	...
	gxmy( 3, toAccessorArray( x ), 2, 1, toAccessorArray( y ), 1, 0 );
	t.deepEqual( y, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports a `y` offset (accessors)', function test( t ) {
	...
	gxmy( 3, toAccessorArray( x ), 1, 0, toAccessorArray( y ), 1, 1 );
	t.deepEqual( y, expected, 'returns expected value' );
	t.end();
});
